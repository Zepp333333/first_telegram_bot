...
class IronStartEvent(TriathlonEvent):
    ...
    def pupulate_event_data(self, event):
        self.add_data(
            url=ironstar_base_url + event.attrs.get('href'),
            ...
            # image_url=ironstar_base_url + re.search(
            #               r'(\/.*)\);',
            #               event.find('div', {'class': 'image'})['style']
            #           ).group(1)
        )
        ...
        image_url = ironstar_base_url + re.search(r'(\/.*)\);', div_url).group(1)
